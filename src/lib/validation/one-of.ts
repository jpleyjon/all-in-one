...
/**
 * Builds a validator that requires at least one predicate to pass.
 *
 * @param validators Validators to compose.
 * @returns Validator that passes when any predicate passes.
 * @throws {RangeError} If no validators are provided.
 * @throws {TypeError} If any validator is not a function.
 */
export function oneOf(...validators: ValidationPredicate[]): ValidationPredicate {
  ...
  return (value: unknown): value is unknown => validators.some((validator) => validator(value));
}
